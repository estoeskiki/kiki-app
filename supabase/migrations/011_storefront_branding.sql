-- ============================================================================
-- Migration: 011_storefront_branding.sql
-- Description: Per-restaurant/food-court welcome background (order-web's
-- Welcome screen) and per-restaurant logo (shown on food-court directory
-- cards). Mirrors the existing organizations.welcome_bg_url pattern
-- (005_org_welcome_bg.sql) at the restaurant/food-court level, since a web
-- storefront is entered per-location, not per-organization.
-- ============================================================================

ALTER TABLE restaurants ADD COLUMN IF NOT EXISTS welcome_bg_url text;
ALTER TABLE restaurants ADD COLUMN IF NOT EXISTS logo_url text;
ALTER TABLE food_courts ADD COLUMN IF NOT EXISTS welcome_bg_url text;

-- Re-expose the new columns through get_public_storefront (SECURITY DEFINER,
-- anon-callable) — no secret columns involved, safe to add.
  ...
  IF v_restaurant_id IS NOT NULL THEN
    SELECT jsonb_build_object(
      ...
      'restaurant', jsonb_build_object(
        ...
        'logo_url', r.logo_url, 'welcome_bg_url', r.welcome_bg_url
      ),
      'table_id', v_table.id,
  ...
  ELSE
    SELECT jsonb_build_object(
      ...
      'food_court', jsonb_build_object(
        ...
        'welcome_bg_url', fc.welcome_bg_url
      ),
      'restaurants', (
